...
{
  ofstream out_deb;
  out_deb.open("../text/OUT_DEB.txt",ios::app);
  out_deb<<s<<endl;
  out_deb.close();
  cout<<s<<" ";
...
void create_debruijn(graph *G,long int n)
{
  cout<<"create_debruijn.cpp :: _____FUNCTION: create_debruijn_____"<<endl;
   ...
   string str;
   fstream infile("../text/Output_dna_segments.txt");
   long int count=0;
   long int prev=0;
   infile>>str;
   vertices[str]=count;
   ...
   while(infile)
   {
     ...
     cout<<str<<endl;
     if(vertices.find(str)==vertices.end())//this string has not been previously seen so make a new vertex for it
     {
       vertices[str]=count;//maps the string to the number of the vertex since graph is built on numbers
       rev_vert[count]=str;
       count++;
...
void convert_dna()
{
  fstream inp("../text/OUT_DEB.txt");
  ofstream output_seq("../text/SEQU.txt",ios::app);
  string str;
  long int count;
  long int t=0;
  while(inp>>str&&!(t>=80))
